...
const FEED = (process.env.DATA_FEED || 'sip').trim().toLowerCase();
// Validated like every other config value: an unrecognized feed would otherwise skip the
// feed fallback entirely and silently price the whole universe off the Yahoo fallback.
if (!['sip', 'iex'].includes(FEED)) throw new Error(`CONFIG: DATA_FEED='${process.env.DATA_FEED}' (use sip|iex)`);
...
async function alpacaBars(symbols, feed) {
  ...
  for (let page = 0; page < 20; page++) {
    ...
    for (const [sym, bars] of Object.entries(j.bars || {})) {
      ...
      if (!rec) continue;
      for (const b of bars) {
        if (Number.isFinite(b.c) && b.c > 0) {   // a 0/negative close would poison RSI/momentum
          rec.closes.push(b.c);
...
async function yahooBars(sym) {
  ...
  for (let i = 0; i < series.length; i++) {
    if (Number.isFinite(series[i]) && series[i] > 0) {
...
// A symbol whose primary-source series is shorter than minBars falls through to the
// fallback source instead of being accepted short.
export async function getDailyBars(symbols, minBars = 1) {
  const bars = new Map();
  const failures = [];
    ...
    for (const feed of FEED === 'iex' ? ['iex'] : [FEED, 'iex']) {
      try {
        ...
        for (const [sym, rec] of got) {
          if (rec.closes.length >= minBars) bars.set(sym, { ...rec, source: `alpaca-${feed}` });
        }
  ...
  for (const sym of remaining) {
    try {
      const rec = await yahooBars(sym);
      if (rec.closes.length < minBars) {
        failures.push({ sym, error: `only ${rec.closes.length} bars from fallback (need ${minBars})` });
        continue;
      }
      bars.set(sym, rec);
    } catch (e) {
