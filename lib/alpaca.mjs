...
import { parseNum } from './config.mjs';
...
// reserved by a not-yet-canceled stop 403s with held_for_orders, so poll until the order
// reaches a terminal state. Returns that status — the caller must check it, because
// "terminal" is not the same as "the shares are free": a stop that FILLED means the
// position is already gone, and selling again would open a short.
const TERMINAL = new Set(['canceled', 'filled', 'expired', 'rejected', 'replaced', 'done_for_day']);
export const FREED = new Set(['canceled', 'expired', 'rejected']); // shares released, safe to sell

// 20s of polling, not 4s: on 2026-08-27 a paper-account stop cancel was still
// 'pending_cancel' after 4.5s, which blocked the SMH exit and left the position
// stopless for ~21h once the cancel finally landed. The job budget is 10 minutes.
// Overridable so the test suite can exercise the give-up path in milliseconds rather
// than 20 real seconds (same role as ALPACA_DATA_BASE/YAHOO_BASE in lib/data.mjs).
export const CANCEL_WAIT_TRIES = parseNum('CANCEL_WAIT_TRIES', 20, { min: 1, max: 600 });
export const CANCEL_WAIT_DELAY_MS = parseNum('CANCEL_WAIT_DELAY_MS', 1000, { min: 1, max: 10_000 });
export const CANCEL_WAIT_SECONDS = (CANCEL_WAIT_TRIES * CANCEL_WAIT_DELAY_MS) / 1000;

export async function cancelAndWait(id, { tries = CANCEL_WAIT_TRIES, delayMs = CANCEL_WAIT_DELAY_MS } = {}) {
  ...
  for (let i = 0; i < tries; i++) {
    ...
    try {
      ...
      if (e.status === 404) return 'canceled';  // gone entirely
      throw e;                                   // a transient API error is NOT a confirmation
    }
    if (TERMINAL.has(o.status)) return o.status;
    await new Promise(res => setTimeout(res, delayMs));
  }
  return 'pending_cancel'; // never confirmed — caller must treat as not freed
}
...
// 'replaced' is deliberately NOT dead: a replaced order has a live successor, so retrying
// under a new id would add a second live order alongside it.
const DEAD = new Set(['rejected', 'canceled', 'expired']);
