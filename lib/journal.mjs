...
// ET date the CURRENT position in `sym` was opened, per the committed journal — or null
// when that cannot be established. Scanning backwards, the most recent journaled event for
// the symbol decides: if it is a SELL, the journal's last knowledge is "flat", so whatever
// is held now was opened without being journaled (a lost journal push, or an order the
// broker accepted while we saw an error). Returning a stale buy date there would make a
// one-day-old position look months old and trip the time stop instantly, so we return null
// and the caller treats the age as unknown.
export function lastBuyDate(bot, sym) {
  const runs = readRuns();
  for (let i = runs.length - 1; i >= 0; i--) {
    const r = runs[i];
    if (r.bot !== bot || !Array.isArray(r.orders)) continue;
    const forSym = r.orders.filter(o => o.symbol === sym);
    if (!forSym.length) continue;
    if (forSym.some(o => o.side === 'sell')) return null;
    if (forSym.some(o => o.side === 'buy')) return r.date_et ?? null;
  }
  return null;
}
