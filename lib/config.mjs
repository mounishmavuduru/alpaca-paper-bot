...
export function assertDisjoint(aName, a, bName, b, env = process.env) {
  ...
  if (overlap.length && !parseBool('ALLOW_UNIVERSE_OVERLAP', false, env)) {
    throw new Error(`CONFIG: ${aName} and ${bName} overlap (${overlap.join(', ')}). ` +
      `Two strategies must not manage the same symbols on one account — they will trade ` +
      `each other's positions. FIX: remove ${overlap.join('/')} from either ${aName} or ` +
      `${bName}. (Expert override: repo Variable ALLOW_UNIVERSE_OVERLAP=true.)`);
  }
  return overlap;
}
